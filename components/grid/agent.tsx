    ...
    return (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
            {agents?.map((agent: IAgent) => (
                <Link key={agent.name} href={`/${orgSlug}/agents/${agent.slug}`} prefetch>
                    <Card
                        key={agent.id}
                            ...
                            <CardHeader className="flex-none pb-4">
                                <div className="mb-4 flex items-start justify-between">
                                    <div className="relative">
                                        ...
                                        <div className="absolute -bottom-1 -right-1 h-4 w-4 rounded-full bg-green-500 ring-2 ring-white"></div>
                                    </div>
                                    <div className="rounded-full bg-blue-50 px-2 py-1 text-[10px] font-semibold tracking-wider text-blue-600">
                                        AGENT
                                    </div>
                                </div>
                                <CardTitle className="line-clamp-2 text-lg font-semibold leading-tight tracking-tight text-gray-900 group-hover:text-blue-900">
                                    {agent.name}
                                </CardTitle>
                            </CardHeader>
