...
import { AnalyticsFilters, CONVERSATION_TYPE } from "@/lib/api/actions/organisation/get-organisation-analytics";

interface AnalyticsTabContentProps {
    className?: string;
}

export default function AnalyticsTabContent({ className }: AnalyticsTabContentProps) {
    ...
    const calculateChartConfig = (dataLength: number) => {
        ...
        return {
            width: finalWidth,
            interval,
            barSize: Math.min(maxBarWidth, Math.max(minBarWidth, (finalWidth - 100) / dataLength)),
        };
    };
    ...
    const chartData = analyticsData?.dailyBreakdown.map(day => ({
                ...
                <Card>
                    <CardContent className="flex items-center justify-center py-8">
                        <div className="text-center">
                            <div className="text-destructive text-lg font-medium">Error loading analytics</div>
    ...
    const uniqueSources = [...new Set(analyticsData.dailyBreakdown.flatMap(() => ["website", "phone", "email", "referral"]))];
        ...
        <>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Select
                            ...
                            onValueChange={(value: string) => handleFilterChange('agent', value)}
                        >
                            <SelectTrigger className="w-32">
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {/* Core Metrics */}
                    <Card>
                        <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2 flex-grow">
                            <CardTitle className="text-sm font-medium">Total Conversations</CardTitle>
                            <MessageSquare className="h-4 w-4 text-muted-foreground" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-2xl font-bold">{analyticsData.conversationAnalytics.totalConversations.toLocaleString()}</div>
                            <p className="text-xs text-muted-foreground">
                                Used credits: {analyticsData.usedConversationCredits.toLocaleString()}
                            </p>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{analyticsData.conversationAnalytics.totalCalls.toLocaleString()}</div>
                            <p className="text-xs text-muted-foreground">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{analyticsData.conversationAnalytics.totalLeadsGenerated.toLocaleString()}</div>
                            <p className="text-xs text-muted-foreground">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{analyticsData.conversationAnalytics.averageConversationLength.toFixed(1)}</div>
                            <p className="text-xs text-muted-foreground">
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2 flex-grow">
                            <CardTitle className="text-sm font-medium">
                                Remaining {analyticsData.creditsPlan === "CONVERSATION" ? "Conversation" : "Message"} Credits
                            </CardTitle>
                            <TrendingUp className="h-4 w-4 text-muted-foreground" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-2xl font-bold">{analyticsData.remainingConversationCredits.toLocaleString()}</div>
                            <p className="text-xs text-muted-foreground">
                                Available credits
                            </p>
                        </CardContent>
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2 flex-grow">
                            <CardTitle className="text-sm font-medium">Remaining Call Credits</CardTitle>
                            <Phone className="h-4 w-4 text-muted-foreground" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-2xl font-bold">{analyticsData.remainingCallCredits.toLocaleString()}</div>
                            <p className="text-xs text-muted-foreground">
                                Available minutes
                            </p>
                        </CardContent>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{analyticsData.conversationAnalytics.averageCallLength.toFixed(1)}</div>
                            <p className="text-xs text-muted-foreground">
                                Minutes per call
                            </p>
                        </CardContent>
